...
`package.json` 中 `name` 以 `@` 开头，说明是个 `group` ，需要先注册一个 `group` ，否则后面没法发布和获取
...
- `lerna diff`
  检查做出的修改，前提是有提交仓库或者有`git commit`
...
1. `lerna version`
   *`lerna version` bump 版本号后，会提交到远程仓库，并在远程上产生一个tag。`lerna version` 成功之后，是不能`lerna publish` 发布成功的，会说没有修改*
...
- 调用 `core/lerna/index.js` `main(argv)` 方法
- 在 `main(argv)` 方法，根据 `option` `ls` 执行对应的`.command() `：`commands/list/commond.js`
