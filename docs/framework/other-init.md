...
最后我们再来看一下 `assertType` 函数的实现，`assertType` 函数定义在 `assertProp` 函数的下方，如下：
...
function assertType (value: any, type: Function): {
  ...
  let valid 
  const expectedType = getType(type)  // 省略...
  return {
    valid,   
    expectedType  }
}
...
上面代码中首先检测方法名字 `key` 是否已经在组件实例对象 `vm` 中有了定义，并且该名字 `key` 为保留的属性名，什么是保留的属性名呢？根据 [isReserved](http://localhost:8080/vue-design/appendix/core-util.html#isreserved) 函数可知以字符 `$` 或 `_` 开头的名字为保留名，如果这两个条件都成立，说明你定义的方法与 `Vue` 原生提供的内置方法冲突，比如：
...
在 `resolveInject` 函数体内所有代码都被包含在了一个 `if` 语句块中：
...
这段代码定义了 `result` 常量，该常量的值为通过 `Object.create(null)` 创建的空对象，并且 `result` 常量的值将来会作为返回值被返回。接着定义了 `keys` 常量，它的值是一个数组，即由 `inject` 选项对象所有键名组成的数组，在 [Vue 选项的规范化](http://localhost:8080/vue-design/art/4vue-normalize.html#规范化-inject（normalizeinject）) 一节中我们讲到了 `inject` 选项被规范化后将会是一个对象，并且该对象必然会包含 `from` 属性。例如如果你的 `inject` 选项是一个字符串数组：
